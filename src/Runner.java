public class Runner {
    public static void main(String[] args){
        System.out.println("Hello World");
        System.out.println("Hello Worlx");
        System.out.println("Hello Worxx");
        System.out.println("Hello Woxxx");
        System.out.println("Hello Wxxxx");
        System.out.println("Hello xxxxx");
        System.out.println("Hellx xxxxx");
        System.out.println("Helxx xxxxx");
        System.out.println("Hexxx xxxxx");
        System.out.println("Hxxxx xxxxx");
        System.out.println("xxxxx xxxxx");
    }
}

